...
LETTER_VALUES = {"A": 1,
                 ...
                 "H": 4,
                 "I": 1,
                 "J": 10, # why is J the value of 1??? changed to 10.
                 "K": 5,
                 "L": 1,
                 "M": 3,
                 "N": 1,
...
class Bag:
    ...
    def initialize_bag(self):
        ...
        self.add_to_bag(Tile("J", LETTER_VALUES), 1) # why so many?, should be 1, not 9?
    ...
    def check_word(self):
        ...
        if self.word != "":
            ...
            if "#" in self.word:
                ...
                self.word = self.word[:self.word.index("#")] + blank_tile_val.upper() + self.word[(self.word.index("#")+1):]  # ORIGINAL LINE OF CODE REPLACED HERE
            ...
            if blank_tile_val != "":
                needed_tiles = needed_tiles[:needed_tiles.index(blank_tile_val)] + needed_tiles[needed_tiles.index(blank_tile_val)+1:] # ORIGINAL LINE OF CODE REPLACED HERE
...
def turn(player, board, bag):
    ...
    if (skipped_turns < 6) or (player.rack.get_rack_length() == 0 and bag.get_remaining_tiles() == 0):
        ...
        print()
        for p in players:
            print(p.get_name() + "'s Letter Rack: " + p.get_rack_str())
        ###################################################
        ...
        while word.check_word() != True:
            ...
            row = input("Row number: ")
            if (col == "" or row == "") or (col not in [str(x) for x in range(15)] or row not in [str(x) for x in range(15)]):
                word.set_location([-1, -1]) # ORIGINAL LINE OF CODE REPLACED HERE
            else:
        ...
        if word.get_word() == "":
            ...
            board.place_word(word.get_word(), word.location, word.direction, player) # ORIGINAL LINE OF CODE REPLACED HERE
        ...
        ### ADD 20% CHANCE TO SKIP NEXT PLAYER'S TURN
        skipChance = random.randint(0,100)
        if skipChance > 20: # don't skip next player's turn. play on as usual.
            print("Noone was skipped.")

            #Gets the next player. if the player index is not the last one, then simply increment; otherwise, if it is the last one, then the next player is player[0]
        ...
        else: # skip next player's turn
            print("SKIPPED")

            if players.index(player) < (len(players)-2):
                player = players[players.index(player)+2]
            else:
                player = players[(players.index(player)+len(players)) % 2]
                round_number += 1 # I think this is OK?

            print("It is now " + player.get_name() + "'s turn.")
        #############################################
